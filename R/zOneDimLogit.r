oneDimLogit <- function(data, index, weights, thresh = 0.0001, lambdas=NULL, beta.naught = rep(0,ncol(data$x)), inner.iter = 100, outer.iter = 100, outer.thresh = 0.0001, gamma = 0.8, step = 1, reset = 10, alpha = 0.95, min.frac= 0.05, nlam = 20, verbose = FALSE){
  if(is.null(lambdas)){
    lambdas <- betterPathCalc(data=data, index=index, weights=weights, alpha=alpha, min.frac=min.frac, nlam=nlam, type="logit")
  }
